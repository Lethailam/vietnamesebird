    ...
    private void UpdatePopupUI()
{
    ...
    if (_unlockRequirementText != null)
    {
        ...
        if (isUnlocked)
        {
            _unlockRequirementText.text = "High Score: " + highScore + "  |  Ready to use";
        }
        else
        {
            int requiredScore = _currentPreviewIndex * _scorePerUnlock;
            int remain = Mathf.Max(0, requiredScore - highScore);
