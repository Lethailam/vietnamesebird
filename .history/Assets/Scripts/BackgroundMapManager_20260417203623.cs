...
public class BackgroundMapManager : MonoBehaviour
{
    ...
    private void UpdatePopupUI()
    {
        ...
        if (_unlockRequirementText != null)
        {
            if (isCurrent)
            {
                ...
                {
                    _unlockRequirementText.text = "You have reached the final location";
                }
                else
                {
                    int nextTarget = (currentIndex + 1) * _scorePerUnlock;
                    int remaining = Mathf.Max(0, nextTarget - highScore);
                    _unlockRequirementText.text = remaining + " more points to reach " + _locations[currentIndex + 1].locationName;
                }
            }
            else if (isNext)
            {
                ...
                _unlockRequirementText.text = remaining + " more points to unlock " + data.locationName;
            }
            else if (isPassed)
            {
                _unlockRequirementText.text = "You have already passed this location";
            }
            else
            {
                int requiredScore = _currentPreviewIndex * _scorePerUnlock;
                _unlockRequirementText.text = "Need " + requiredScore + " points to reach " + data.locationName;
            }
        }
    }
    ...
    public void RefreshUnlockUI()
    {
        ...
        if (_progressText != null)
        {
            if (currentIndex >= _locations.Length - 1)
            {
                _progressText.text = "Current: " + _locations[currentIndex].locationName + "\nJourney completed";
            }
            else
            {
                int nextTarget = (currentIndex + 1) * _scorePerUnlock;
                ...
                _progressText.text =
                    "Current: " + _locations[currentIndex].locationName +
                    "\n" + remaining + " more points to reach " + _locations[currentIndex + 1].locationName;
            }
        }
    }
