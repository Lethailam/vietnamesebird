...
public class BackgroundMapManager : MonoBehaviour
{
    ...
    private void UpdatePopupUI()
    {
        ...
        bool isNext = nextIndex.HasValue && _currentPreviewIndex == nextIndex.Value;
        ...
        if (_unlockRequirementText != null)
        {
            if (isCurrent)
            {
                ...
                {
                    int nextRequiredScore = GetRequiredScoreForLocation(currentIndex + 1);
                    int remaining = Mathf.Max(0, nextRequiredScore - highScore);

                    _unlockRequirementText.text =
                        remaining + " MORE POINTS\nTO REACH " +
                        ToUpperSafe(_locations[currentIndex + 1].locationName);
                }
            }
            else if (isNext)
            {
                int requiredScore = GetRequiredScoreForLocation(_currentPreviewIndex);
    ...
    private int? GetNextLocationIndex(int highScore)
    {
        ...
        if (current >= _locations.Length - 1)
            return null;

        return current + 1;
    }

    private int GetRequiredScoreForLocation(int locationIndex)
    {
        return Mathf.Max(0, locationIndex) * _scorePerUnlock;
    }
