...
public class BackgroundMapManager : MonoBehaviour
{
    ...
    private readonly Dictionary<int, int> _routeOrderToArrayIndex = new Dictionary<int, int>();

    private void Awake()
    {
        Instance = this;
        AutoAssignRouteOrder();
        BuildRouteLookup();
    }
    ...
    private void AutoAssignRouteOrder()
    {
        for (int i = 0; i < _locations.Length; i++)
        {
            string nameUpper = (_locations[i].locationName ?? "").Trim().ToUpper();

            switch (nameUpper)
            {
                case "HA NOI":
                    _locations[i].routeOrder = 0;
                    break;
                case "NGHE AN":
                    _locations[i].routeOrder = 1;
                    break;
                case "DONG HOI":
                    _locations[i].routeOrder = 2;
                    break;
                case "DONG HA":
                    _locations[i].routeOrder = 3;
                    break;
                case "HUE":
                    _locations[i].routeOrder = 4;
                    break;
                case "DA NANG":
                    _locations[i].routeOrder = 5;
                    break;
                case "NHA TRANG":
                    _locations[i].routeOrder = 6;
                    break;
                case "SAI GON":
                    _locations[i].routeOrder = 7;
                    break;
                default:
                    Debug.LogWarning("UNKNOWN LOCATION NAME: " + _locations[i].locationName);
                    break;
            }
        }
    }
    ...
    private void BuildRouteLookup()
    {
        ...
        for (int i = 0; i < _locations.Length; i++)
        {
            ...
            Debug.Log("LOCATION: " + _locations[i].locationName + " | ROUTE ORDER: " + order);

            if (_routeOrderToArrayIndex.ContainsKey(order))
            {
                Debug.LogError("DUPLICATE ROUTE ORDER: " + order + " | " + _locations[i].locationName);
                continue;
            }
    ...
    private void UpdatePopupUI()
    {
        ...
        if (_stateText != null)
        {
            if (isCurrent) _stateText.text = "CURRENT";
            else if (isNext) _stateText.text = "NEXT";
            else if (isPassed) _stateText.text = "COMPLETED";
            else _stateText.text = "LOCKED";
        }
    ...
    private int? GetNextRouteOrder(int highScore)
    {
        ...
        if (currentOrder >= GetMaxRouteOrder()) return null;
